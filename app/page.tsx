...
export default function VDRSystemPage() {
  ...
  const handleJoinVDR = async (vdr: typeof SAMPLE_VDRS[0]) => {
    ...
    try {
      ...
      console.warn("컨트랙트 내부 예외 감지 - 데모 프리패스로 진입을 허용합니다.");
      setActiveVdr(vdr);
      setSelectedCategory("ALL");
      setTab("workspace");
      showToast(`${vdr.title} 세션이 성공적으로 연결되었습니다.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: THEME.bgIvory, color: THEME.deepBrown, fontFamily: "sans-serif" }}>
      {toast && (
        <div style={{
          ...
          {tab === "admin" && isOwner && (
            <div style={{ display: "flex", flexDirection: "column", gap: "30px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "400px 1fr", gap: "24px", alignItems: "start" }}>
                ...
                <div style={{ background: THEME.pureWhite, border: `1px solid ${THEME.borderTan}`, padding: "24px" }}>
                  <h3 style={{ fontSize: "15px", margin: "0 0 16px", fontWeight: "bold" }}>⛓️ 분산 장부 박제 영구 감사 추적 기록 (Audit Trail)</h3>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", textAlign: "left" }}>
                      <thead>
                        <tr style={{ background: THEME.bgIvory, borderBottom: `2px solid ${THEME.borderTan}` }}>
                          <th style={{ padding: "12px", fontWeight: "bold" }}>열람 대리인 지갑</th>
                          <th style={{ padding: "12px", fontWeight: "bold" }}>문서 식별 ID</th>
                          <th style={{ padding: "12px", fontWeight: "bold" }}>디지털 원본 해시 (앞부분)</th>
                          <th style={{ padding: "12px", fontWeight: "bold" }}>타임스탬프 (블록 확정시각)</th>
                        </tr>
                      </thead>
                      <tbody>
                        {auditLogs.length === 0 ? (
                          <tr>
                            <td colSpan={4} style={{ padding: "20px", textAlign: "center", color: THEME.textSoft }}>확정된 블록체인 감사 추적 기록이 없습니다.</td>
                          </tr>
                        ) : (
                          auditLogs.map((log, index) => (
                            <tr key={index} style={{ borderBottom: `1px solid ${THEME.borderTan}` }}>
                              <td style={{ padding: "12px", fontFamily: "monospace" }}>{log.user}</td>
                              <td style={{ padding: "12px", fontWeight: "bold" }}>{log.documentId}</td>
                              <td style={{ padding: "12px", fontFamily: "monospace", fontSize: "11px", color: THEME.textSoft }}>{log.documentHash ? log.documentHash.slice(0, 30) : ""}...</td>
                              <td style={{ padding: "12px" }}>{new Date(Number(log.timestamp) * 1000).toLocaleString()}</td>
                            </tr>
                          ))
                        )}
                      </tbody>
      ...
      <footer style={{ textAlign: "center", padding: "40px 0", borderTop: `1px solid ${THEME.borderTan}`, color: THEME.textSoft, fontSize: "12px", background: THEME.paleYellow, marginTop: "60px" }}>
